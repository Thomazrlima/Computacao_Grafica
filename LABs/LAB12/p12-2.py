...
class Example(Base):
    ...
    Z controls camera, X controls harmonic, C controls gaita de foles
    """
    def initialize(self):
        ...
        self.camera_rig.set_position([0, 1.75, 4.5])
        ...
        cima_peq_material = LambertMaterial(texture=cima_peq_texture, number_of_light_sources=2, use_shadow=True)
        sphere_material = LambertMaterial(texture=sphere_texture, number_of_light_sources=2, use_shadow=True)
        sphere2_material = LambertMaterial(texture=sphere2_texture, number_of_light_sources=2, use_shadow=True)
        baixo_material = LambertMaterial(texture=baixo_texture, number_of_light_sources=2, use_shadow=True)
        cima_material = LambertMaterial(texture=cima_texture, number_of_light_sources=2, use_shadow=True)
        baixo_peq_material = LambertMaterial(texture=baixo_peq_texture, number_of_light_sources=2, use_shadow=True)
        sphere2_baixo_material = LambertMaterial(texture=sphere2_baixo_texture, number_of_light_sources=2, use_shadow=True)
        sphere_baixo_material = LambertMaterial(texture=sphere_baixo_texture, number_of_light_sources=2, use_shadow=True)
        main_material = LambertMaterial(texture=main_texture, number_of_light_sources=2, use_shadow=True)
        ...
        branco_texture = Texture(file_name="images/branco.jpg")
        preto_texture = Texture(file_name="images/preto.jpg")
        escuro_texture = Texture(file_name="images/escuro.jpg")

        vermelho_material = LambertMaterial(texture=vermelho_texture, number_of_light_sources=2, use_shadow=True)
        branco_material = LambertMaterial(texture=branco_texture, number_of_light_sources=2, use_shadow=True)
        preto_material = LambertMaterial(texture=preto_texture, number_of_light_sources=2, use_shadow=True)
        escuro_material = LambertMaterial(texture=escuro_texture, number_of_light_sources=2, use_shadow=True)

        mesh_vermelho = Mesh(geom_vermelho, vermelho_material)
        mesh_branco = Mesh(geom_branco, branco_material)
        mesh_preto = Mesh(geom_preto, preto_material)
        mesh_escuro = Mesh(geom_escuro, escuro_material)
        ...
        grass_material = LambertMaterial(
            texture=Texture(file_name="images/solo.jpg"), number_of_light_sources=2, use_shadow=True, property_dict={"repeatUV": [50, 50]}
        )
        ...
        self.directional_light = DirectionalLight(color=[1, 1, 0], direction=[-1, -1, 0])
        ...
        self.directional_light.set_position([2.75, 5, -0.5])
        ...
        print("Camera movement: WASDRF(move), QE(turn), TG(look).\nPress Z for camera, X for harmonic, or C for gaita de foles.")

    def update(self):
        self.directional_light.rotate_y(0.013, False)
