...
class Example(Base):
    ...
    def initialize(self):
        ...
        self.scene = Scene()
        self.camera = Camera(aspect_ratio=800/800)
        ...
        fragment_shader_code = """
            ...
            void main()
            {
                ...
                vec4 color1 = texture2D(texture1, uvNoise);
                vec4 color2 = texture2D(texture2, uvNoise);
                float s = abs(sin(time));
                fragColor = s * color1 + (1.0 - s) * color2;
            }
        """
        grid_texture = Texture("images/Messi.jpg")
        crate_texture = Texture("images/CR7.webp")
    ...
    def update(self):
        self.blend_material.uniform_dict["time"].data += self.delta_time*3
